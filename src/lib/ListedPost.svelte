...
<a href="/p/{post.slug}" class="p-2 bg-slate-200 w-full rounded-lg" on:mouseover={() => dispatch('select')} on:focus={() => dispatch('select')}>
    <img src="https://l2dgt-blog-db.school.izmichael.com/api/files/posts/{post.id}/{post.image}" class="float-left rounded-lg h-20 aspect-video mr-3" title={post.imageAlt} alt={post.imageAlt} />
