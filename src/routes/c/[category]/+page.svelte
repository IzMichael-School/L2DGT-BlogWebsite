<script lang="ts">
    ...
    let selected: Post | undefined = data.posts[0];
</script>
...
<div class="w-full h-full max-h-full overflow-hidden flex flex-row justify-center items-center p-10">
    <div class="flex flex-col justify-start items-center flex-1 mr-5 h-full max-w-[50%] bg-slate-200 rounded-lg shadow-lg px-5 pt-5 pb-2">
        {#if selected?.id}
            <img src="https://l2dgt-blog-db.school.izmichael.com/api/files/posts/{selected.id}/{selected.image}" class="rounded-lg" alt={selected.imageAlt} />
            ...
            <button class="p-2 hover:bg-blue-500 rounded-lg">Read the Post -></button>
        {/if}
    </div>
